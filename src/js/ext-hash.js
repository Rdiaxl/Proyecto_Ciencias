const ExtTransformacion = (() => {
  ...
  function setMethod(method) {
    ...
    const select = document.getElementById('extTransMethod');
  ...
  function generate() {
    M = Number(document.getElementById('extTransM').value);
    B = Number(document.getElementById('extTransB').value);
    if(M < 2) M = 2;
    if(B < 1) B = 1;
    ...
    renderRam(null);
    updateCalc('—', '—', '—');
    document.getElementById('extTransLog').innerHTML = '';
    document.getElementById('extTransStatus').textContent = `Disco preparado: ${M} cubetas de capacidad ${B}. Método: ${currentMethod.toUpperCase()}`;
  ...
  function updateCalc(key, opText, resultText) {
    document.getElementById('etCalcKey').textContent = key;
    document.getElementById('etCalcOp').innerHTML = opText;
    document.getElementById('etCalcResult').textContent = resultText;
    
    document.querySelectorAll('#extTransCalc .calc-box').forEach(b => {
  ...
  function renderDisk(activeBucket = -1, loadedBucket = -1) {
    const wrap = document.getElementById('extTransDisk');
  ...
  function renderRam(chunk, target = null, activeIdx = -1) {
    const wrap = document.getElementById('extTransRam');
    if (!chunk) {
  ...
  async function processHash(mode) {
    ...
    const keyInput = document.getElementById('extTransKey');
    const k = Number(keyInput.value);
    const status = document.getElementById('extTransStatus');
    const log = document.getElementById('extTransLog');
  ...
  // Inicialización segura para que la pestaña activa y el disco se dibujen al cargar
  setTimeout(() => {
    setMethod('modulo');
  }, 100);
